    ...
    novacom.Resolver.prototype = {
        ...
        load: function(next) {
            ...
            function _replaceBuiltinSshKey(next) {
                ...
                var builtinPrvKeyForEmul = path.join(__dirname, "../../files/conf/", 'webos_emul');
            ...
            function _adjustList(next) {
                ...
                var defaultTarget = "emulator";

                //count targes does not have "default field"
                var defaultFields = inDevices.filter(function(device) {
        ...
        _addOne: function(inDevice, next) {
            ...
            var systemCmd = cliData.getCommandService();
        ...
        setTargetName: function(target, next) {
            ...
            if (!name) {
                ...
                if (usbDevices.length > 0 && usbDevices[0].name) {
                    ...
                    var defaultTargets = this.devices.filter(function(device) {
                        return (device.default && device.default === true );
                    });
                    if (defaultTargets.length > 1) {
                        return next(new Error("multiple devices set to default target, please reset device list"));
